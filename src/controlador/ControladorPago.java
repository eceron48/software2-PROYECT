...
public class ControladorPago implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
			...
			if (!vistaPago.txtCedula.getText().isEmpty()) {
				...
				try {
					...
					for (Residente pe : listas) {
						...
						this.vistaPago.tbpersonas.addRow(
								new Object[] { pe.getId(), pe.getCedula(), pe.getNombre(), pe.getCuota().getTotal(),
										pe.getCuota().getUltimoPago(), pe.getCuota().getObservacion() });
						suma = suma + pe.getCuota().getTotal();
						vistaPago.txtNombre.setText(pe.getNombre());
					}
				} catch ( NullPointerException | SQLException ex) {
						ex.printStackTrace();
					JOptionPane.showMessageDialog(null, "el documento ingresado no fue encontrado intente de nuevo ",
							null, 3);
				}
		...
		case "mostrar pagos":
			...
			try {
				...
				for (Residente p : listado) {
					...
					this.vistaPago.tbpersonas.addRow(new Object[] { p.getId(), p.getCedula(), p.getNombre(),
							p.getCuota().getTotal(), p.getCuota().getUltimoPago(), p.getCuota().getObservacion() });
		...
		case "registrar":
			if (!vistaPago.rdbEfectivo.isSelected() && !vistaPago.rdbDebito.isSelected()
					&& !vistaPago.rbtCredito.isSelected()) {
				JOptionPane.showMessageDialog(null, "seleccione una forma de pago ", null, 2);
			} else {
